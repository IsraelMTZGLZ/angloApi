...
CREATE TABLE Tb_CartaCondicionalOIncondicional(
	idDocumento int not null AUTO_INCREMENT PRIMARY KEY,
	urlDocumento text not null,
	typeDocumento varchar(15),
	extDocumento varchar(10),
...
CREATE TABLE Tb_RecomendacionCartaCOI(
  idRecomendacion int primary key auto_increment,
  descripcion text,
  CD timestamp default current_timestamp,
  LU timestamp default current_timestamp on update current_timestamp,
...
CREATE OR REPLACE VIEW Vw_DocumentosVA AS SELECT idAspirante, statusDocumento
from Tb_formsolicitudVAcademicoAspirante as f, Tb_Aspirantes as a
where f.fkAspirante = a.idAspirante and f.statusDocumento = 'Aceptado';
...
from Tb_Aspirante_Ingles_C_A_I as ae, Tb_TipoAlojamientoCurso as ta, Tb_TipoCursos as c, Tb_Aspirantes as a
where ae.fkAspirante = a.idAspirante  and ae.fkCurso = c.idTipoCurso and ae.fkAlojamiento = ta.idTipoAlojamiento  ;
