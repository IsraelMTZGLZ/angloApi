...
class Api extends REST_Controller {
    ...
    function campamento_get(){
        $id = $this->get('id');
        if($id){
             $response = array(
                "status"=>"success",
                "message"=> '',
                "data"=>$this->DAO->selectEnt('Tb_Campamentos',array('idCampamento'=>$id)),
            );
        }else{
            $response = array(
                "status"=>"success",
                "message"=> '',
                "data"=>$this->DAO->selectEnt('Tb_Campamentos'),
            );
        }
        $this->response($response,200);
    }

    function campamento_post(){
        $data = $this->post();
        ...
        if(count($data) == 0 || count($data) > 3){
            ...
            $this->form_validation->set_rules('abreviacion','Abreviacion','required');
             ...
             if($this->form_validation->run()==FALSE){
               ...
               $data=array(
                   "nombreCampamento"=>$this->post('nombre'),
                   "abreviacionCampamento"=>$this->post('abreviacion')
               );
               $response = $this->DAO->insertData('Tb_Campamentos',$data);
