...
class Api extends REST_Controller {
    ...
    public function permisosAgentes_post(){
        $data = $this->post();
        ...
        if(count($data) == 0 || count($data) > 3){
            $response = array(
                ...
                "data"=>null,
                "validations"=>array(
                    "permiso"=>"El permiso id es requerido",
                    "agente"=>"El agente id es requerido"
                )
            );
        }else{
            $this->form_validation->set_data($data);
            $this->form_validation->set_rules('permiso','Permiso id','required');
            $this->form_validation->set_rules('agente','Agente id','required');
             ...
             if($this->form_validation->run()==FALSE){
               ...
               $data=array(
                   "fkPermiso"=>$this->post('permiso'),
                   "fkAgente"=>$this->post('agente')
               );
               $response = $this->DAO->insertData('Tb_Permisos_Agentes',$data);
    ...
    public function permisosAgentes_delete(){
        $id = $this->get('id');
      if ($id) {
        $IdExists = $this->DAO->selectEntity('Tb_Permisos_Agentes',array('idPermisoAgente'=>$id),TRUE);

        if($IdExists){
          $response = $this->DAO->deleteData('Tb_Permisos_Agentes',array('idPermisoAgente'=>$id));
        }else{
          $response = array(
            "status"=>"error",
            "status_code"=>409,
    ...
    function permisosAgenteByAgente_get(){
        $id=$this->get('id');
        if (count($this->get())>1) {
            $response = array(
                "status" => "error",
            ...
            if ($id) {
                $data = $this->DAO->selectEntity('Vw_Permiso',array('agente'=>$id),false);
            }
            if ($data) {
                $response = array(
                    "status" => "success",
