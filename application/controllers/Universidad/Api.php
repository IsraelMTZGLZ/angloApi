...
class Api extends REST_Controller {
  ...
  function universidadByFacultad_get(){
    $id=$this->get('id');
    if (count($this->get())>1) {
      $response = array(
          "status" => "error",
      ...
      if ($id) {
          $data = $this->DAO->selectEntity('Vw_Uni',array('idFacultad'=>$id),false);
      }else{
          $data= null;
      }
      if ($data) {
  ...
  $this->response($response,200);
    ...
    public function universidadOrdenByInstitucionWhereInstitucion_get()
    {
        $id = $this->get('id');
        if (count($this->get())>1) {
            $response = array(
            ...
            $query = $this->db->query("select * from Vw_Uni where idInstitucion=".$id." group by idInstitucion")->result();
